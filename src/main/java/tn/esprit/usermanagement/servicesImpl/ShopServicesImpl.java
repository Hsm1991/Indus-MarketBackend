...
public class ShopServicesImpl implements ShopServices {
    ...
    private localisationRepo locationRepo;
    ...
    @Override
        public Shop addShopAndAffectToUser(Shop s, List<MultipartFile> files, float longitude , float latitude) throws Exception {
            s.setUser(authenticationService.currentlyAuthenticatedUser());

            localisation address = addressService.reverseGeoccode(longitude , latitude);
            locationRepo.save(address);
            s.setAddress(address);
            s.setAdresse(address.getCity()+" "+address.getState());
    ...
    @Override
    public Shop editShop(Shop s, List<MultipartFile> files , float longitude , float latitude) throws IOException {
        if(shopRepo.getReferenceById(s.getIdShop()).getUser().getId()== authenticationService.currentlyAuthenticatedUser().getId()){
           ...
           localisation newAdresse = addressService.reverseGeoccode(longitude,latitude);
           newAdresse.setId(shopRepo.getReferenceById(s.getIdShop()).getAddress().getId());
           s.setAddress(locationRepo.save(newAdresse));
           s.setUser(authenticationService.currentlyAuthenticatedUser());
    ...
    @Override
    public void updateSopStat(int id) {
        Shop s = shopRepo.findById(id).get();
        s.setValidated(true);
        shopRepo.saveAndFlush(s);
    }
